...
export PATH="/usr/local/opt/curl/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"
...
# curl:
# editorconfig: it configs your editor
# ffmpeg: encode mp4/mov for use with gifsicle
# gh:
# gifsicle: convert movies to gif
# gnu-sed:
# gnupg:
# gnutls:
# googler: search Google via CLI
# imagemagick: eventually I will need this for something
# lerna: CLI for Lerna monorepos
# libdvdcss: makes handbreak capable of ripping DVDs
# neovim: supercharged vim
# nettle:
# nvm:
# openssl@1.1: ...
# pinentry-mac: secret input
# pyenv: python version manager
# readline: command-line editing
# ripgrep: rg is a faster grep alternative
# shfmt: autoformat shell script source code
# speedtest-cli: run a speedtest via the command line
# switchaudio-osx: switch the audio device via the command line
# tesseract: optical character recognition (OCR)
# the_silver_searcher: really fast searching
# tig: feature-rich GUI for git log
# trash: move file/directory to trash
# tree: list out directories in text as trees
# wget:
# yarn: alternative, better node package manager
# zlib:
brew install bat delta curl nvm openssl@1.1 pyenv readline shfmt switchaudio-osx tree watchman wget yarn

echo "installing node (via nvm)"
[ -s "$NVM_DIR/nvm.sh" ] && \. "$NVM_DIR/nvm.sh"  # This loads nvm
nvm install 8 && nvm install-latest-npm
nvm install 12 --alias=default && nvm install-latest-npm
nvm use default
...
brew install --cask firefox google-drive openvpn-connect
# qlcolorcode qlmarkdown qlstephen qlvideo qmoji quicklook-json webpquicklook \

echo "Generating a new SSH key for GitHub"
# ssh-keygen -t ed25519 -C "1915925+sbolel@users.noreply.github.com" -f ~/.ssh/id_ed25519
eval "$(ssh-agent -s)"
touch ~/.ssh/config
echo "Host *\n AddKeysToAgent yes\n UseKeychain yes\n IdentityFile ~/.ssh/id_ed25519" | tee ~/.ssh/config
ssh-add -K ~/.ssh/id_ed25519
echo "NOTE: run 'pbcopy < ~/.ssh/id_ed25519.pub' and paste that into GitHub"

echo "cloning dotfiles"
git clone git@github.com:sbolel/.dotfiles.git "${HOME}/.dotfiles"
...
# Follow the keyboard focus while zoomed in # "could not write com.apple.univer.."
...
# defaults write NSGlobalDomain AppleLanguages -array "en_US" "en" "es"
